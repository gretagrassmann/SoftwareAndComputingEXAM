...
#the training and testing data are downoaded and unzipped
...
#function to initialize the weights' values in the convolutional and dense layers
...
def node_average_model(input, params, filters=None, dropout_keep_prob=1.0, trainable=True):
    ...
    nh_sizes = tf.expand_dims(tf.count_nonzero(nh_indices + 1, axis=1, dtype=tf.float32),
                              -1)  # for fixed number of neighbors, -1 is a pad value
    if params is None:
        # create new weights
...
#the classification has to be performed on an average of the results for the ligan-receptor and receptor-ligand pairs
