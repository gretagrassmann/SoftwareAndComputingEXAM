...
def node_average_model(input, params, filters=None, dropout_keep_prob=1.0, trainable=True):
    """This function defines the operations performed in the convolutional layers: starting from some initial values for
       ...
       https://raw.github.com/gretagrassmann/SoftwareAndComputingEXAM/master/GCN.pdf.
              ...
              Returns:
                  h : new features of edges and vertices. To each one of them a different value for each one of the applied
...
def merge(input):
    """This function unifies the output (of dimension [minibatch size,#filter]) of the two convolutional
...
def build_graph_conv_model(in_nv_dims, in_ne_dims, in_nhood_size):
    """This function defines how the variable resulting from the architecture of the model are produced. The architecture
       ...
       and the right one for the receptor proteins). This two "legs" have independent weights.
       The merging of their two outputs in a single tensor, which is then fed into two dense layers.
       The final dense layers associate to each pair's combination a single value.
    ...
    with tf.name_scope(name):
        #this second branch has some new weights, independent from the left one
    ...
    with tf.name_scope(name):
        #this second branch has some new weights, independently from the left one
        output, _ = node_average_model(input2, None, filters=256, dropout_keep_prob=0.5)
        input2 = output, in_edge2, in_hood_indices2
